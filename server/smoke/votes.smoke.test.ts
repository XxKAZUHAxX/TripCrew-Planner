...
describe('votes', () => {
    ...
    it('computes Borda scores and tracks change count', async () => {
        const { api } = h;
        ...
        const tokyo = (
            await api('POST', `${tBase}/destinations`, { token: alice, body: { name: 'Tokyo' } })
        ).data.destination;
        const bali = (
            await api('POST', `${tBase}/destinations`, { token: alice, body: { name: 'Bali' } })
        ).data.destination;
        const oslo = (
            await api('POST', `${tBase}/destinations`, { token: alice, body: { name: 'Oslo' } })
        ).data.destination;
        ...
        const byId = Object.fromEntries(
            tally.data.scores.map((s: { destId: string; score: number }) => [s.destId, s.score])
        );
