...
try {
  const reg = await api('POST', '/api/auth/register', {
    body: { name: 'Alice', email: 'alice@example.com', password: 'pw12345' },
  });
  ...
  const dup = await api('POST', '/api/auth/register', {
    body: { name: 'Alice2', email: 'alice@example.com', password: 'pw12345' },
  });
  ...
  assert(meRes.status === 200 && meRes.data.user.name === 'Alice', '/me returns current user');
