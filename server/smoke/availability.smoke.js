...
try {
  ...
  const save = await api('PUT', aBase, { token: alice, body: { dates: ['2025-07-04', '2025-07-05'] } });
  assert(save.status === 200 && save.data.availability.dates.length === 2, 'alice saves dates');
  ...
  // Empty heatmap shape.
  const trip2 = (await api('POST', '/api/trips', { token: alice, body: { title: 'Empty' } })).data.trip;
  const emptyHeat = await api('GET', `/api/trips/${trip2._id}/availability/heatmap`, { token: alice });
