...
try {
  ...
  assert([t._id, b._id].includes(spin.data.winningDestinationId), 'winner is one of the slices');
  assert(spin.data.status === 'decided', 'status becomes decided');
  assert(typeof spin.data.winnerIndex === 'number', 'winnerIndex returned for deterministic animation');
