...
export async function submitVote(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
        ...
        if (!Array.isArray(ranking)) {
            res.status(400).json({ message: 'ranking must be an array of destination ids' });
            return;
        }
        ...
        for (const id of ranking) {
            if (!validIds.has(String(id))) {
                res.status(400).json({ message: `Invalid destination id in ranking: ${id}` });
                return;
            }
        }
        const uniqueCount = new Set(ranking.map(String)).size;
        if (uniqueCount !== ranking.length) {
            res.status(400).json({ message: 'ranking must not contain duplicates' });
            return;
        }
...
export async function getTally(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
