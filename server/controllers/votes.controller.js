    ...
    try {
        ...
        if (!Array.isArray(ranking)) {
            return res
                .status(400)
                .json({
                    message: "ranking must be an array of destination ids",
                });
        }
        // Validate every id belongs to this trip.
        const tripDestinations = await Destination.find({
            tripId: req.trip._id,
        }).select("_id");
        ...
        for (const id of ranking) {
            if (!validIds.has(String(id))) {
                return res
                    .status(400)
                    .json({
                        message: `Invalid destination id in ranking: ${id}`,
                    });
            }
        }
        ...
        if (uniqueCount !== ranking.length) {
            return res
                .status(400)
                .json({ message: "ranking must not contain duplicates" });
        }
