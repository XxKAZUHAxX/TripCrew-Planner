...
export async function addTask(req, res, next) {
  try {
    ...
    res.status(201).json({ task: { id: task._id, label: task.label, createdBy: task.createdBy } });
  } catch (err) {
    next(err);
  }
}
...
export async function deleteTask(req, res, next) {
  try {
    ...
    if (!isAuthor && !isCreator) {
      return res.status(403).json({ message: 'Only the task author or trip creator can delete it' });
    }
    task.deleteOne();
    await req.trip.save();
    res.json({ ok: true });
